...
def test_decode_msg():
    ...
    with raises(
        ValueError,
        match='No header found in message',
    ):
        decode_msg(b'{"testing": true}')
    with raises(
        ValueError,
        match='Header must contain Content-Length',
    ):
        decode_msg(b'Content-Type: json-rpc\r\n\r\n{"testing": true}')
    with raises(ValueError, match='Content-Length does not match actual content length'):
